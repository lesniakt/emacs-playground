...
** Magit
This comes before the evil-collection block due to the after hook in evil-collection needing magit already.
...
*** C/C++ Mode
lsp-mode will auto-detect clangd if it's in your $PATH (server-side if using TRAMP/ssh)
